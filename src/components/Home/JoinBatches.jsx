...
const JoinBatches = () => {
  ...
  return (
    <div className="mt-24 py-32 px-10 flex flex-col items-center justify-between space-y-8" id="join">
      ...
      <div className="flex flex-col items-start justify-between space-x-3 md:flex-row">
        {
          batchData.map((batch) => (
            <div className="flex flex-col items-center justify-between space-y-2 shadow-sm py-10 px-2 w-[380px]" key={batch.title}>
              {batch.icon}
              <h1 className="uppercase font-bold text-2xl text-white">{batch.title}</h1>
              <Link
                to={batch.linkTo}
                onClick={scrollToTop}
